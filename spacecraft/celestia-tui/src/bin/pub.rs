...
impl<'a> Buffer<'a> {
    ...
    async fn enable_realtime_edit(&mut self, session: Session, user: String) -> io::Result<()> {
        ...
        if self.realtime {
            ...
            if self.realtime_publisher.is_none() {
                Self::handle_text_block_queries(Arc::clone(&self.text), user.clone(), self.id.clone(), session.clone(), self.task_tx.clone()).await;
    ...
    async fn handle_text_block_queries(text: Arc<Mutex<Text>>, user: String, id: Ulid, session: Session, task_tx: mpsc::Sender<JoinHandle<()>>) {
        let (tx, rx) = flume::bounded(32);
        let key_expression = user + "/editor/encodedtext";
        let handle = tokio::spawn(async move {
            ...
            while let Ok(query) = rx.recv_async().await {
                ...
                if block_id == id {
                    let serialized = serde_json::to_string(&text.lock().await.encoded_text_from()).unwrap();
                    query.reply(key_expression.as_str(), format!("{}", serialized)).await.unwrap();
                }
            }
        });
